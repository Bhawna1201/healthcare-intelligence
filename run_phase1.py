...
def main():
    ...
    if args.reset:
        ...
        for name in ["clinicaltrials", "fda_shortages", "webmd_reviews",
    ...
    if "reviews" in active_scrapers:
        ...
        results["WebMD Reviews"] = run_scraper(
            "WebMD Reviews", run_reviews,
            drugs=drugs, max_pages_per_drug=2 if args.test else 15
        )

    # ── 4. NADAC Drug Prices (CMS Government Database) ───────────────
    if "prices" in active_scrapers:
        from scrapers.nadac_scraper import run as run_prices
        results["NADAC Prices"] = run_scraper(
            "NADAC Prices", run_prices,
            drugs=drugs,
        )
